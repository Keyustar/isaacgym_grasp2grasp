...
class nowrist(VecTask):
    ...
    def _init_buffers(self):
        ...
        #print("net_contact_force_tensor:",net_contact_force_tensor)
        ...
        print(f"hand_goal_dof_pos shape!!!!!!!!!!!!!!!!: {self.hand_goal_dof_pos.shape}")
        #提取所有环境中目标物体的自由度状态
        ...
        self.recording_save_path = os.path.join(
            recording_dir,
            f'{timestamp_str}')
        # gym_methods = [method for method in dir(self.gym) if 'contact' in method.lower()]
        # print("Available contact-related methods:")
        # for method in gym_methods:
        #     print(f"  - {method}")
    ...
    def check_termination(self):
        ...
        # 记录调试信息
        self.extras["joint_diff_min"] = joint_diff.min().item()
        self.extras["joint_diff_max"] = joint_diff.max().item()
        self.extras["pos_diff_min"] = pos_diff.min().item()
        self.extras["pos_diff_max"] = pos_diff.max().item()
        self.extras["rot_diff_min"] = rot_diff.min().item()
        self.extras["rot_diff_max"] = rot_diff.max().item()
        ...
        # 计算接触力
        # if not hasattr(self, 'fingertip_contact_forces'):
        #     net_contact_force_tensor = self.gym.acquire_net_contact_force_tensor(self.sim)
        #     # # print("net_contact_force_tensor:", net_contact_force_tensor)
        #     print("net_contact_force_tensor.shape:", net_contact_force_tensor.shape)
        #     net_contact_forces = gymtorch.wrap_tensor(net_contact_force_tensor)
        #     # # print("net_contact_forces:", net_contact_forces)
        #     # # print("net_contact_forces.shape:", net_contact_forces.shape)
        #     bodies_per_env = net_contact_forces.shape[0] // self.num_envs
        #     contact_forces_reshaped = net_contact_forces.view(self.num_envs, bodies_per_env, 3)  
        #     print("contact_forces_reshaped:", contact_forces_reshaped.shape)
        #     self.fingertip_contact_forces = contact_forces_reshaped[:, self.force_sensor_handles]
        net_contact_force_tensor = self.gym.acquire_net_contact_force_tensor(self.sim)
        # # print("net_contact_force_tensor:", net_contact_force_tensor)
        #print("net_contact_force_tensor.shape:", net_contact_force_tensor.shape)
        net_contact_forces = gymtorch.wrap_tensor(net_contact_force_tensor)
        ...
        #print("contact_forces_reshaped:", contact_forces_reshaped.shape)
        self.fingertip_contact_forces = contact_forces_reshaped[:, self.force_sensor_handles]            
        #print("self.fingertip_contact_forces:", self.fingertip_contact_forces)
        fingertip_force_magnitudes = torch.norm(self.fingertip_contact_forces, dim=2)
        fingertip_has_contact = fingertip_force_magnitudes > 0 #0.5N
        #print("fingertip_has_contact:",fingertip_has_contact)
        if not hasattr(self, 'no_contact_counter'):
            self.no_contact_counter = torch.zeros(self.num_envs, device=self.device)
        if not hasattr(self, 'contact_counter'):
            self.contact_counter = torch.zeros(self.num_envs, device=self.device)        
        num_contacting_fingertips = fingertip_has_contact.sum(dim=1)  # [8]   
        # 检查当前步是否有接触
        contact_this_step = (num_contacting_fingertips > 2)
        no_contact_this_step = (num_contacting_fingertips == 0)
        #print("no_contact_this_step:",no_contact_this_step)
        # 更新计数器
        self.contact_counter = torch.where(contact_this_step, 
                                            self.contact_counter + 1, 
                                            torch.zeros_like(self.contact_counter))
        self.no_contact_counter = torch.where(no_contact_this_step, 
                                            self.no_contact_counter + 1, 
                                            torch.zeros_like(self.no_contact_counter))
        # 当连续超过15步没有接触时，设置contact_buff为True
        self.contact_buff = (self.no_contact_counter >= 15)
        # 计算每个环境中接触的指尖数量
     
        # self.contact_buff = (num_contacting_fingertips == 0)
        # 重置条件
        ...
        self.reset_buf = timeout_buf | self.dropped_buf | self.contact_buff #| self.success_buf
    ...
    def reset_current_grasp_pose(self, env_ids):
        """向量化版本（更高效）"""
        # 调用批量加载函数
        _, _, obj_pos, obj_rot, hand_joints = self.load_grasp_poses_batch(
            self.current_grasp_idx[env_ids]
        )
        # 批量处理手部关节状态（直接使用返回的张量）
        self.hand_dof_init_pos[env_ids, ] = hand_joints  # [B,N] 直接赋值
    ...
    def reset_goal_grasp_pose(self, goal_env_ids):
        ...
        # 批量处理手部关节状态（直接使用返回的张量）
        self.hand_goal_dof_init_pos[goal_env_ids, ] = hand_joints  # [B,N] 直接赋值
    ...
    def reset_object_states(self, env_ids):
        ...
        self.resetted_object_states[:, 9] = 0.03  # z方向线速度
    ...
    def reset_idx(self, env_ids, goal_env_ids):
        ...
        if len(env_ids) > 0:
            ...
            # 重置接触计数器
            if hasattr(self, 'contact_counter'):
                self.contact_counter[env_ids] = 0
            if hasattr(self, 'no_contact_counter'):
                self.no_contact_counter[env_ids] = 0
                # self.contact_buff[env_ids] = False            
    ...
    def compute_observations(self):
        ...
        if torch.isnan(self.obs_buf).any() or torch.isinf(self.obs_buf).any():
            print("警告: 观测中发现异常值!")
            nan_count = torch.isnan(self.obs_buf).sum().item()
            inf_count = torch.isinf(self.obs_buf).sum().item()
            print(f"NaN 数量: {nan_count}, Inf 数量: {inf_count}")
            
            # 打印异常值的位置和对应的观测函数
            if nan_count > 0:
                nan_positions = torch.where(torch.isnan(self.obs_buf))
                print(f"NaN 位置: env {nan_positions[0][:5].tolist()}, obs_dim {nan_positions[1][:5].tolist()}")
            
            # # 替换异常值为0
            # self.obs_buf = torch.where(torch.isnan(self.obs_buf) | torch.isinf(self.obs_buf), 
            #                         torch.zeros_like(self.obs_buf), self.obs_buf)
    ...
    def _create_envs(self):
        ...
        print("加载hand_asset",hand_asset_file)
        ...
        print("self.num_hand_actuators:",self.num_hand_actuators)
        self.num_hand_tendons = self.gym.get_asset_tendon_count(hand_asset)
        print("self.num_hand_tendons:",self.num_hand_tendons)
        # set up tendons
        ...
        print("self.hand_dof_lower_limits:",self.hand_dof_lower_limits)
        self.hand_dof_upper_limits = to_torch(
            self.hand_dof_upper_limits, device=self.device
        )
        print("self.hand_dof_upper_limits:",self.hand_dof_upper_limits)
        ...
        print("self.actuated_dof_lower_limits:",self.actuated_dof_lower_limits)                
        print("self.actuated_dof_upper_limits:",self.actuated_dof_upper_limits)
        ...
        print("force_sensor_handles:",force_sensor_handles)
        ...
        for i in range(self.num_envs):
            ...
            # hand_dof_pos = [0.0] * 22 # 初始化24维列表
            hand_dof_pos = hand_joints_state
            ...
            # hand_goal_dof_pos = [0.0] * 22 # 初始化24维列表
            hand_goal_dof_pos = goal_hand_joints_state
            ...
            self.gym.set_actor_scale(env_ptr, goal_obj_handle, goal_obj_scale)
            ...
            num_bodies_in_env = self.gym.get_env_rigid_body_count(env_ptr)
            print(f"环境 {i}: {num_bodies_in_env} 个刚体")
            if self.cfg["env"]["aggregate_mode"]:
    ...
    def select_goal_by_nn_batch(self, current_grasp_indices, joint_min_dist=0.1, joint_max_dist=1.0, 
        ...
        # 返回结果张量
        return goal_indices
    def select_goal_by_nn_batch_gpu_parallel(self, current_grasp_indices, alpha=0.5, sample_size=1000, top_k=5, max_rot_distance=0.5):
        """
        使用完全并行的张量运算进行高效的批量最近邻搜索（无for循环）
        取消上下限阈值，直接在combined_distance最小的top_k个中随机选择
        添加旋转距离阈值限制
        
        Args:
            current_grasp_indices: 当前手势的索引张量 [batch_size]
            alpha: 关节角度距离的权重系数 (0-1)，用于平衡关节距离和物体距离
            sample_size: 随机下采样的数量
            top_k: 从距离最小的前k个候选中随机选择
            max_rot_distance: 最大允许的旋转距离阈值 (弧度)
        
        Returns:
            goal_indices: 选择的目标手势索引张量 [batch_size]
        """
        def compute_rotation_distance(quat1, quat2):
            """
            计算两个四元数之间的旋转距离
            quat1: [batch_size, 1, 4] 或 [batch_size, 4]
            quat2: [1, num_candidates, 4] 或 [num_candidates, 4]
            返回: [batch_size, num_candidates] 或对应维度
            """
            if quat1.dim() == 2:
                quat1 = quat1.unsqueeze(1)
            if quat2.dim() == 2:
                quat2 = quat2.unsqueeze(0)
            
            # 计算点积
            dot_product = torch.sum(quat1 * quat2, dim=-1)
            
            # 处理四元数双重覆盖 (q 和 -q 表示相同旋转)
            dot_product = torch.abs(dot_product)
            
            # 确保数值稳定性，避免acos输入超出[0,1]范围
            dot_product = torch.clamp(dot_product, 0.0, 1.0)
            
            # 计算角度距离
            rot_distances = 2 * torch.acos(dot_product)
        ...
        # 计算旋转距离 - 使用四元数
        rot_distances = compute_rotation_distance(current_obj_rot_quat, candidate_obj_rot_quat)   # [batch_size, num_candidates]        
        ...
        # 创建旋转距离阈值掩码 [batch_size, num_candidates]
        rot_distance_mask = rot_distances <= max_rot_distance  # [batch_size, num_candidates]
        
        # 计算综合距离分数 [batch_size, num_candidates]
        # 使用alpha参数来平衡关节距离和物体距离
        obj_distance = pos_distances + rot_distances
        combined_distance = alpha * joint_distances + (1 - alpha) * obj_distance
        
        # 创建综合掩码：必须同时满足所有条件
        final_mask = exclude_current_mask & valid_mask & rot_distance_mask  # [batch_size, num_candidates]
        
        # 将无效候选的距离设为无穷大
        masked_distance = combined_distance.clone()
        masked_distance[~final_mask] = float('inf')
        
        # 检查每个batch的有效候选数量
        valid_candidates_per_batch = torch.sum(final_mask, dim=1)  # [batch_size]
        min_valid_candidates = torch.min(valid_candidates_per_batch).item()
        
        # 动态调整top_k以避免超出有效候选数量
        actual_top_k = min(top_k, min_valid_candidates, num_candidates)
        ...
        if actual_top_k > 0:
            # 有足够的有效候选，使用topk选择
            top_k_values, top_k_indices = torch.topk(masked_distance, k=actual_top_k, dim=1, largest=False)
            
            # 为每个batch生成随机索引，避免for循环
            # 但由于每个batch的有效候选数量可能不同，仍需要逐个处理
            for batch_idx in range(batch_size):
                valid_top_k = top_k_values[batch_idx] < float('inf')
                if valid_top_k.any():
                    # 从有效的top_k中随机选择一个
                    valid_indices = top_k_indices[batch_idx][valid_top_k]
                    selected_idx = valid_indices[torch.randint(0, len(valid_indices), (1,), device=self.device)].item()
                    goal_indices[batch_idx] = candidate_indices[selected_idx]
                else:
                    # 没有有效候选，选择combined_distance最小的非当前索引
                    # 重新计算掩码，只排除当前索引和无效手势，不考虑旋转距离限制
                    relaxed_mask = exclude_current_mask[batch_idx] & valid_mask[batch_idx]
                    relaxed_distance = combined_distance[batch_idx].clone()
                    relaxed_distance[~relaxed_mask] = float('inf')
                    
                    # 找到最小距离的索引
                    min_distance_idx = torch.argmin(relaxed_distance).item()
                    goal_indices[batch_idx] = candidate_indices[min_distance_idx]
        else:
            # 没有有效候选，对所有batch选择combined_distance最小的非当前索引
            for batch_idx in range(batch_size):
                # 重新计算掩码，只排除当前索引和无效手势，不考虑旋转距离限制
                relaxed_mask = exclude_current_mask[batch_idx] & valid_mask[batch_idx]
                relaxed_distance = combined_distance[batch_idx].clone()
        ...
        return goal_indices
    # def select_goal_by_nn_batch_gpu_parallel(self, current_grasp_indices, alpha=0.5, sample_size=1000, top_k=5):
    #     """
    #     使用完全并行的张量运算进行高效的批量最近邻搜索（无for循环）
    #     取消上下限阈值，直接在combined_distance最小的top_k个中随机选择
    ...
    #     # dot_product = torch.clamp(torch.abs(dot_product), 0, 1)
    #     # print("current_obj_rot_quat.shape, candidate_obj_rot_quat.shape",current_obj_rot_quat.shape, candidate_obj_rot_quat.shape)
    #     # quat_diff = quat_mul(current_obj_rot_quat, candidate_obj_rot_quat)
    #     # rot_diff = 2.0 * torch.asin(
    #     #     torch.clamp(torch.norm(quat_diff[:, 0:3], p=2, dim=-1), max=1.0)
    #     # )
    #     rot_distances = compute_rotation_distance(current_obj_rot_quat, candidate_obj_rot_quat)   # [batch_size, num_candidates]        
    ...
    #     # 计算综合距离分数 [batch_size, num_candidates]
    #     obj_distance = pos_distances + 1 * rot_distances
    #     combined_distance = joint_distances + obj_distance
        
    #     # 将无效候选的距离设为无穷大
    #     masked_distance = combined_distance.clone()
    #     masked_distance[~exclude_current_mask] = float('inf')
    #     masked_distance[~valid_mask] = float('inf')
        
    #     # 对每个batch，找到距离最小的top_k个候选
    #     top_k_values, top_k_indices = torch.topk(masked_distance, k=min(top_k, num_candidates), dim=1, largest=False)
        
    #     # 初始化结果张量
    #     goal_indices = torch.zeros(batch_size, dtype=torch.long, device=self.device)
        
    #     # 对每个batch，从top_k中随机选择一个
    #     for batch_idx in range(batch_size):
    #         # 检查是否有有效候选
    #         valid_top_k = top_k_values[batch_idx] < float('inf')
    #         if valid_top_k.any():
    ...
    # def select_goal_by_nn_batch_gpu_parallel_origin(self, current_grasp_indices, joint_min_dist=0, joint_max_dist=1.0, 
    #                                         obj_pos_dist=0.1, obj_rot_dist=0.5, alpha=0.7, sample_size=500):
    #     """
    #     使用完全并行的张量运算进行高效的批量最近邻搜索（无for循环）
    ...
    #         # 计算综合距离分数 [batch_size, num_candidates]
    #         obj_distance = pos_distances + 0.2 * rot_distances
    #         combined_distance = alpha * joint_distances + (1 - alpha) * obj_distance * 10   
    #         # 对于每个batch，选择最优候选
    #         goal_indices = self._select_candidates_min(
    ...
    #     return goal_indices
    # def _select_candidates_min(self, combined_distance, all_constraints, exclude_current_mask, 
    #                                         valid_mask, candidate_indices):
    #     """
    #     并行选择最小距离候选（在满足约束的候选中选择距离最小的）
    ...
    #     # 将无效候选的距离设为无穷大，这样它们不会被选中
    #     masked_distance = combined_distance.clone()
    #     masked_distance[~selection_mask] = float('inf')
        
    #     # 选择距离最小的候选
    #     selected_positions = torch.argmin(masked_distance, dim=1)  # [batch_size]
    #     goal_indices = candidate_indices[selected_positions]
    ...
    def load_grasp_poses(self, grasp_index=None):
        ...
        # 提取物体缩放系数
        obj_scale = selected_grasp.get('scale', 0.1)
        # 提取手部位姿数据
    ...
    #     """
    #     self.extras["obj_dist"] = torch.norm(self.object_pos - self.goal_pos, p=2, dim=-1).mean().item()
    ...
    #         torch.clamp(torch.norm(quat_diff[:, 0:3], p=2, dim=-1), max=1.0)
    #     )
    #     self.extras["rot_diff"] = rot_dist.mean().item()
    #     rot_eps = 0.00001
    ...
    def _reward_obj_pos_rot_combined(self):
        ...
        # 姿态误差（忽略绕物体自身Z轴的扭转，仅比较Z轴方向夹角）
        obj_z = quat_apply(self.object_rot, self.z_unit_tensor)   # [N,3]
        goal_z = quat_apply(self.goal_rot, self.z_unit_tensor)    # [N,3]
        cos_theta = torch.clamp(torch.sum(obj_z * goal_z, dim=-1), -1.0, 1.0)
        rot_diff = torch.acos(cos_theta)  # 仅倾斜角，不含绕Z轴扭转
        ...
        self.extras["combined"] = combined.mean().item()
    ...
    # ), p=2, dim=1)
        ...
        reward = joint_diff**2
        #reward = torch.exp(-joint_diff * 5.0)
    ...
    def _reward_contact_force_penalty(self):
        return self.no_contact_counter
    ...
    #     #     obj_id = 17
    #     #     query_list = [obj_id * hash_num + 2, obj_id * hash_num + 6, obj_id * hash_num + 10, obj_id * hash_num + 15, obj_id * hash_num + 21]
    #     #     return len(np.intersect1d(query_list, li))
    #     # # assert self.device == 'cpu'
    #     # contacts = [self.gym.acquire_contact_tensor(env) for env in self.envs]
    #     # contact_list = [list_intersect(np.unique([c[0] * 10000 + c[1] for c in contact]), 10000) for contact in contacts]
    #     # contact_condition = to_torch(contact_list, device=self.device)
    #     # has_contact = contact_condition <= 2
    # # 获取净接触力张量
    #     net_contact_force_tensor = self.gym.acquire_net_contact_force_tensor(self.sim)
    #     net_contact_forces = gymtorch.wrap_tensor(net_contact_force_tensor)
    #     # print("net_contact_forces.shape:", net_contact_forces.shape)  
    #     bodies_per_env = net_contact_forces.shape[0] // self.num_envs
    #     # 重塑为 [num_envs, bodies_per_env, 3]
    #     net_contact_forces_reshaped = net_contact_forces.view(self.num_envs, bodies_per_env, 3)  
    #     # print("net_contact_forces_reshaped.shape:", net_contact_forces_reshaped.shape)
 
    #     hand_contact_forces = net_contact_forces_reshaped[:, self.force_sensor_handles]   
    #     fingertip_force_magnitudes = torch.norm(hand_contact_forces, dim=2)
    #     fingertip_has_contact = fingertip_force_magnitudes > 0.005 #0.5N# [8, 5]
    #     # 计算每个环境中接触的指尖数量
    #     num_contacting_fingertips = fingertip_has_contact.sum(dim=1)  # [8]
    #     has_sufficient_contact = num_contacting_fingertips <= 2
    #     # 可选：记录调试信息
    #     self.extras["avg_contacting_fingertips"] = num_contacting_fingertips.float().mean().item()
    #     self.extras["sufficient_contact_rate"] = has_sufficient_contact.float().mean().item()    
    #     # object_contact_forces = net_contact_forces_reshaped[:,57]
    #     # print("hand_contact_forces.shape ", hand_contact_forces.shape)
    #     # print("object_contact_forces.shape ", object_contact_forces.shape)  
    #     return has_sufficient_contact.float()
    # def _reward_reorienttask_obj_dist(self):
    #     """
    #         基于物体与目标位置之间的距离给予奖励
    #         (距离越小奖励越高)
    ...
    def _observation_goal_hand_pose_diff(self):
        ...
        """
        return self.hand_dof_pos[:, self.actuated_dof_indices] - self.hand_goal_dof_pos[:, self.actuated_dof_indices]  
        # return unscale(
        #     self.hand_dof_pos[:, self.actuated_dof_indices] - self.hand_goal_dof_pos[:, self.actuated_dof_indices],
        #     self.actuated_dof_lower_limits,
        #     self.actuated_dof_upper_limits,
    ...
    def _observation_fingertip_contact_forces(self):
        """
        Returns the fingertip contact forces
        """
        #print("self.fingertip_contact_forces:",self.fingertip_contact_forces)
        return self.fingertip_contact_forces.view(self.num_envs, -1)
